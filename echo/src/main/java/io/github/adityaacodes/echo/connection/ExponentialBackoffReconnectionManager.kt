...
internal class ExponentialBackoffReconnectionManager(
    ...
    private val suspendAfterMs: Long = 120_000,
) {
    private var reconnectJob: Job? = null
    private var currentAttempt = 0
    private var firstDisconnectTimeMs: Long? = null
    ...
    private fun handleDisconnected(state: ConnectionState.Disconnected) {
        ...
        if (firstDisconnectTimeMs == null) {
            firstDisconnectTimeMs = System.currentTimeMillis()
        }
    ...
    private fun scheduleReconnect(immediate: Boolean) {
        ...
        val isSuspended = firstDisconnectTimeMs?.let {
            System.currentTimeMillis() - it >= suspendAfterMs
        } ?: false
        ...
        reconnectJob = scope.launch {
            ...
            val awareConnection = connection as? ReconnectionAwareConnection
            if (isSuspended) {
                awareConnection?.markSuspended()
            } else {
                awareConnection?.markReconnecting(currentAttempt)
            }

            if (!immediate) {
                val delayMs = if (isSuspended) maxDelayMs else calculateDelayWithJitter(currentAttempt)
                delay(delayMs)
            }
            connection.connect()
        }
