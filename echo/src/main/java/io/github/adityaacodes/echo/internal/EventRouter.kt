...
import io.github.adityaacodes.echo.data.protocol.SubscriptionError
...
internal class EventRouter(private val incomingFrames: SharedFlow<PusherFrame>) {
    ...
    fun channelEvents(channelName: String): Flow<PusherFrame> {
        return incomingFrames.filter { frame ->
            when (frame) {
                ...
                is SubscriptionError -> frame.channel == channelName
                is MemberAdded -> frame.channel == channelName
                is MemberRemoved -> frame.channel == channelName
                is MemberUpdated -> frame.channel == channelName
                else -> false
            }
        }
    }
    ...
    private fun PusherFrame.toEchoEvent(): EchoEvent? {
        return when (this) {
            ...
            is SubscriptionError -> EchoEvent("pusher_internal:subscription_error", this.channel, this.data)
            ...
            is MemberUpdated -> EchoEvent("pusher_internal:member_updated", this.channel, this.data)
